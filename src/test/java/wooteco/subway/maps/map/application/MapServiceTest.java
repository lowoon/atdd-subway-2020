...
public class MapServiceTest {
    ...
    void findPath() {
        ...
        when(lineService.findLinesByIds(any())).thenReturn(Collections.singletonList(lines.get(2)));
        ...
        when(fareService.calculateFare(any(), any(), any())).thenReturn(Fare.base());
    ...
    @Test
    void findPathWithDetailedPath() {
        when(lineService.findLines()).thenReturn(lines);
        when(lineService.findLinesByIds(any())).thenReturn(Collections.singletonList(lines.get(2)));
        when(pathService.findPath(anyList(), anyLong(), anyLong(), any())).thenReturn(subwayPath);
        when(stationService.findStationsByIds(anyList())).thenReturn(stations);
        when(fareService.calculateFare(any(), any(), any())).thenReturn(new Fare(1650));
